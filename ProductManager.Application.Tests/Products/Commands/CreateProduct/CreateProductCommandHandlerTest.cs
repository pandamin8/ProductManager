...
public class CreateProductCommandHandlerTest
{
    ...
    [Fact]
    public async Task Handle_ProductWithManufacturerEmailAlreadyExists_ThrowsConflictException()
    {
        // Arrange
        var product = new Product
            { ManufactureContact = new ManufactureContact() { ManufactureEmail = _command.ManufactureEmail }, };

        var currentUser = new CurrentUser("1", "test@test.com", []);
        _userContextMock.Setup(u => u.GetCurrentUser()).Returns(currentUser);

        _productsRepositoryMock.Setup(r => r.GetByManufacturerEmail(_command.ManufactureEmail))
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ConflictException>(() => commandHandler.Handle(_command, CancellationToken.None));
        _productsRepositoryMock.Verify(r => r.GetByManufacturerEmail(It.IsAny<string>()), Times.Once);
        _productsRepositoryMock.Verify(r => r.CreateAsync(It.IsAny<Product>()), Times.Never);
    }
    
    [Fact]
    public async Task Handle_ProductWithManufacturerPhoneAlreadyExists_ThrowsConflictException()
    {
        // Arrange
        var product = new Product
            { ManufactureContact = new ManufactureContact() { ManufactureEmail = _command.ManufactureEmail }, };
        ...
        _productsRepositoryMock.Setup(r => r.GetByManufacturerPhoneNumber(_command.ManufacturePhone))
            .ReturnsAsync(product);
